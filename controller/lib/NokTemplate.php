 ...
 *   Cambios desde 1.1.4
 *   - Bug Fix: Se reparo un bug existente en el uso de las variables din�micas
 *     mediante el m�todo asignarDinamico(); imposibilitando el uso de m�s de una variable.
 ...
 *
 *   Cambios desde 1.1.0
 *   - Se agrega el m�todo cargarVirtual que toma el contenido de una variable (String)
 ...
 *   - Se reparo un smallBug en la utilizaci�n de bloques anidados, reportado por
 ...
 *   - Se le agrego una bandera al m�todo expandir($variable, $template, $analizar = True), con la cual se puede omitir el analisis del template, en caso de que no sea necesario.
 *
...
class NokTemplate {
    ...
    /** Variable que contiene la definici�n de variables Templates.
     * @access private
     * */
     ...
     * @access public
     * @param string $clave Indica el nombre con el cual se referenciar� al template.
     ...
     * @param string $variable Especifica el nombre de la variable en la cual se almacenar� el contenido del template
        ...
        if (!empty($variable) AND !empty($clave)) {
            ...
            if ($clave{0} == '-') {
                ...
                if ($this->_usarCache != 2) {
                    ...
                    if ($analizar) {
                        // Las lineas mas importantes, donde se realiza la interpolaci�n de variables.
    ...
    function asignarDinamico($variable, $valor = '') {
        ...
        if (!empty($variable) AND $this->_usarCache > 0) {
            ...
            $this->error('Debe activar la cache para utilizar esta funci�n.');
        }
    }
        ...
        if ($this->_usarCache != 2) {
            ...
            if (!empty($bloque) AND !empty($template)) {
                ...
                if (preg_match('/' . $this->_tagsBloqueArray['abrir'] . '[ ]*' . $bloque . '[ ]*' . $this->_tagsBloqueArray['close'] .
                                '(.*)' .
                                $this->_tagsBloqueArray['cerrar'] . '[ ]*' . $bloque . '[ ]*' . $this->_tagsBloqueArray['close'] . '/s', $this->_archivosArray[$template], $stringBloque)) {
    ...
    /* Devuelve la versi�n actual de la clase.
     * NokTemplate::version()
     * @access public
     */
    ...
    function obtenerArchivo($nombreArchivo) {
        ...
        if (file_exists($pathReal)) {
            ...
            $this->error("Archivo template: \"$nombreArchivo \" no v�lido.");
        }
    }
    ...
    function debugger($variable) {
        ...
        if (!isset($this->_variablesArray[$variable])) {
            $this->error("<b>Debug:</b> Variable \"$variable\" definida en el Template, no est� inicializada.");
        } else {
            return $this->_variablesArray[$variable];
        }
    }
    ...
    function setDirectorio($dir = './') {
        //
        $this->_path = $this->checkearPath($dir);
    }
